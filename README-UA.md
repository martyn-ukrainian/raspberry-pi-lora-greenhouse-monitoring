...
Заміряно в маленькому відерці на 1 кг ґрунту, вода доливалась порціями по 50 мл, показник сенсора писався як сире ADC + мВ через окрему стендову прошивку — [`firmware/soil-calibration`](./firmware/soil-calibration).

**v2.0 виграє однозначно**: приблизно у 4 рази швидший прогрів (прямий виграш ресурсу батареї на вузлі зі сном) і десь у 6,6 раза точніше розрізняє вологість, ніж v1.2 — той конструктивно 5-вольтовий, на 3,3 В живленні вузла виходить стиснутий і шумніший.

![Калібрувальна крива v2.0: мл води -> показник сенсора (мВ)](firmware/soil-calibration/data/calibration_curve.png)

Кожен наступний долив рухає показник дедалі менше — сенсор насичується. Модель — геометричне спадання дельти кроку:

```
mV(V) = mV_last + Δ_last · r^((V − V_last) / step)        r = Δ_last / Δ_prev
d(mV)/dV = (Δ_last · ln r / step) · r^((V − V_last) / step)    (чутливість, мВ на мл)
```

Заміряно на `v20a`: `r ≈ 0,35` — наступний рівновеликий (50 мл) крок робить ~35% роботи попереднього. Перший чистий 50-мільний крок рухає показник у ~35 разів сильніше на мілілітр, ніж останній крок на 150 мл.

Повні числа й методика — [`docs/калібрування-ґрунту.md`](./docs/калібрування-ґрунту.md).

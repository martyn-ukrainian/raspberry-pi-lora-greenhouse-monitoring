...
логи з усіма таблицями лишаються в `docs/дослідження/калібрування-ґрунту.md` і
...
Повний розрахунок: `docs/дослідження/power-budget.md`.
...
працювати паралельно з вимірюванням. Деталі — `docs/дослідження/шум-від-wifi.md`.

---

## 2026-08-30 — Витік у deep sleep: 0,94 → 0,01 мА ✅ закрито

**Питання.** Вузол у сні їв **0,94 мА** замість розрахункових 0,135 (заміри
спільноти Heltec для V3). Різниця в сім разів, і це не академічна цікавість:
0,94 мА — підлога, під яку не пірнути жодною оптимізацією прошивки. Вона
визначала стелю всієї платформи — близько чотирьох місяців, і то при нульовому
вікні семплювання.

**Метод.** Бісекція збірками. Окремий проєкт `firmware/power-bisect`, де кожне
середовище відрізняється від сусіднього рівно одним доданим блоком
ініціалізації. Прилад (BSIDE ZT102) весь час у розриві «мінуса» банки, між
кроками не міняється нічого, крім прошивки. Сенсорів на платі нема — щуп
виключений з підозрюваних окремим заміром ще 28.08.

| крок | що в прошивці | струм |
|---|---|---:|
| `idle_awake` | не спить, порожній `loop()` | 39,55 мА |
| `step_a_sleep` | лише `esp_deep_sleep_start()` | 0,84 мА |
| `step_b_radio` | + штатний `radio.sleep()` | 0,70 мА |
| `step_e_ropg` | + усі піни в `INPUT`, холодний сон радіо | **0,01 мА** |
| бойова прошивка | контроль | 0,94 мА |

**Що виявилось.** Бісекція закінчилась на першому ж кроці: прошивка, в якій
немає **нічого**, крім засинання, дала 0,84 мА. Отже витік не в тому, що вузол
робить, а в тому, як він лягає спати. Крок B розділив внески — радіо коштувало
0,14 мА, а **піни, залишені виходами, — 0,69**. Пін, що тримає рівень на вході
вже сплячої мікросхеми, живить її вхідні каскади: тече не ESP32, тече через
SX1262 і SSD1306.

Розв'язок узятий з [ropg/heltec_esp32_lora_v3](https://github.com/ropg/heltec_esp32_lora_v3) —
бібліотеки спільноти саме для цієї плати, де заявлено 24 мкА при пробудженні за
таймером. Перед сном вона переводить усі периферійні піни в `INPUT` і кладе
радіо в холодний сон замість теплого.

**Висновок.** Сон перестав бути стелею платформи. Головним споживачем стало
**вікно семплювання**: 39,55 мА неспання проти 0,01 мА сну — курс обміну
1:4000, тобто одна секунда роботи дорівнює годині сну. Уся подальша економія
живе там, і оптимізація вікна з етапу 3 (7 відліків → 3) з «приємної» стала
єдиною, що ще має значення.

| конфігурація | середній струм | від 3000 мАг |
|---|---:|---:|
| 5 хв, вікно 20,5 с (до правки) | 3,48 мА | 36 діб |
| 15 хв, вікно 20,5 с (до правки) | 1,72 мА | 73 доби |
| **15 хв, вікно 20,5 с (після)** | **0,91 мА** | **137 діб** |
| 15 хв, вікно 9,5 с (після + вікно) | 0,43 мА | 293 доби |

Правку перенесено в бойову прошивку — це версія **`lowpower` 2.0**, і дані,
зняті нею, не слід змішувати з попередніми: змінилась не логіка вимірювання, а
енергетичний профіль, тобто крива розряду 18650 з цього моменту інша.

**Межі висновку.** Числа «діб» стоять на непевному фундаменті: реальні 10 діб
проти розрахункових 36 не пояснені, банку підключали недозарядженою (3,47 В), а
шкала `vbat` жодного разу не звірялась мультиметром. Заміряний тут струм —
надійний; переведення його в дати — ні. Також не з'ясовано, які саме з
п'ятнадцяти пінів течуть: рецепт застосовано цілком.

Розбір: `docs/дослідження/струм-сну.md`, прошивки: `firmware/power-bisect`.
